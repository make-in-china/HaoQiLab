...
const bgpic = 'url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAMUlEQVQ4T2NkYGAQYcAP3uCTZhw1gGGYhAGBZIA/nYDCgBDAm9BGDWAAJyRCgLaBCAAgXwixzAS0pgAAAABJRU5ErkJggg==) left center';
...
export default class SketchPicker
    ...
    onColorSlide = new TouchMove((e, target: HTMLElement) => {
        const pos = this.getPos(e, target);
    ...
    updateColor() {
        ...
        const rgba = `rgba(${this.showColor.R},${this.showColor.G},${this.showColor.B},${this.alpha})`;
        this.cssClass!.updateRule({
            colorboxbg: `background-color:${getRGBString(this.baseColor)};`,
            showcolor: `background-color:${rgba};`,
                ...
                <div EClass="colorbox2">
                    <div>
                        ...
                        <div EClass="alpha showAlphaColor" onMouseDown={this.onAlpha.autoDown}>
                            <div style={{ width: colorSlideWidth, height: 10, background: bgpic }} >
                                <div EClass="slide alphaslidepos" />
                            </div>
                        </div>
                    </div>
                    <div>
                        <div style={{ background: bgpic }}>
                            <div EClass="show showcolor" />
                        </div>
                    </div>
                </div>
                <RenderData
    ...
    getPos(e: TouchMoveEvent<HTMLElement>, target: HTMLElement) {
        let x = 0;
        let y = 0;
        ...
        return { x: (e.targetEvent as any).pageX - x, y: (e.targetEvent as any).pageY - y };
    }
    recalShowColor() {
        const color = getRGBByArea(this.baseColor, { R: 255, G: 255, B: 255 }, colorWidth - this.pos.x, colorWidth);
        this.showColor = getRGBByArea(color, { R: 0, G: 0, B: 0 }, this.pos.y, colorHeight);
